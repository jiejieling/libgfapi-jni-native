...
struct dirent *getdirentFields(JNIEnv *env, jobject lpObject, struct dirent *lpStruct) {
    ...
    lpStruct->d_ino = (uint64_t)(*env)->GetLongField(env, lpObject, direntFc.d_ino);
#if defined(__APPLE__)
    lpStruct->d_seekoff = (uint64_t)(*env)->GetLongField(env, lpObject, direntFc.d_off);
#else
    lpStruct->d_off = (uint64_t)(*env)->GetLongField(env, lpObject, direntFc.d_off);
#endif
...
struct statvfs *getstatvfsFields(JNIEnv *env, jobject lpObject, struct statvfs *lpStruct) {
...
#if !defined(__APPLE__)
    {
        ...
        (*env)->GetIntArrayRegion(env, lpObject1, 0, sizeof(lpStruct->__f_spare) / sizeof(jint),
                                  (jint *) lpStruct->__f_spare);
    }
#endif
    return lpStruct;
}
...
void setstatvfsFields(JNIEnv *env, jobject lpObject, struct statvfs *lpStruct) {
    ...
    {
        jintArray lpObject1 = (jintArray)(*env)->GetObjectField(env, lpObject, statvfsFc.__f_spare);
        (*env)->SetIntArrayRegion(env, lpObject1, 0, sizeof(lpStruct->__f_spare) / sizeof(jint),
                                  (jint *) lpStruct->__f_spare);
    }
#endif
}
